...
export default function OfficeBell({ profile }) {
      ...
      sub = supabase
        ...
        .on('postgres_changes', {
          ...
          try {
            ...
            console.error('Bell handler error:', e)
          }
        })
        .subscribe()
    } catch (e) {
      console.error('Bell setup error:', e)
    }
  ...
  const checkActiveCalls = async () => {
    try {
      ...
      console.error('checkActiveCalls error:', e)
    }
  }
  ...
  const playBell = () => {
    try {
      ...
      playTone(880, 0, 0.6)
      playTone(1100, 0.1, 0.4)
      playTone(880, 0.8, 0.6)
      playTone(1100, 0.9, 0.4)
    } catch (e) {}
  }
  ...
  // Agar koi call nahi toh kuch render mat karo
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '52px' }}>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '13px', fontWeight: '600', letterSpacing: '4px', textTransform: 'uppercase', marginBottom: '16px' }}>
          You're being called
        </div>
        <div style={{ color: 'white', fontSize: '40px', fontWeight: '800', letterSpacing: '-1px', marginBottom: '10px' }}>
          {callerName}
        </div>
      ...
      <div style={{ display: 'flex', gap: '20px' }}>
        <button onClick={() => respond('coming')} style={{
          ...
          cursor: 'pointer', boxShadow: '0 4px 24px rgba(22,163,74,0.5)'
        }}>
          ✅ Coming!
        </button>
        <button onClick={() => respond('dismissed')} style={{
          ...
          color: 'rgba(255,255,255,0.7)', fontSize: '17px', fontWeight: '700',
          cursor: 'pointer'
        }}>
          ✕ Dismiss
        </button>
