...
export default function Tasks({ profile }) {
  ...
  const [pipelineUpdate, setPipelineUpdate] = useState({
    stage: '', percent: 0, note: ''
  })
  ...
  const createPipelineStage = async () => {
    ...
    const { error } = await supabase
      ...
      .insert({ name: newStage.trim(), created_by: profile.id })
    if (!error) {
    ...
    <div className="fade-in">
      ...
      {activeTimer && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <div style={{ color: '#888', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Timer Running
              </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{
              color: '#d71920', fontSize: '24px', fontWeight: '800',
              fontFamily: 'monospace', letterSpacing: '0.05em'
            }}>
              {formatTimer(timerSeconds)}
            </div>
            <button onClick={stopTimer} className="btn btn-danger btn-sm">
              ⏹ Stop
            </button>
          </div>
        </div>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ color: '#888', margin: 0, fontSize: '13px' }}>
            {filtered.length} tasks · {completionRate}% complete
          </p>
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{
            display: 'flex', background: 'white', border: '1px solid #e5e5e5',
      ...
      <div style={{
        ...
        {[
          ...
          { label: 'Done', value: doneTasks, color: '#16a34a' },
          { label: 'Overdue', value: overdueTasks, color: '#d71920' },
      ...
      {loading ? (
              ...
              >
                <div className="kanban-column-header">
                  ...
                  <div style={{
                    ...
                    borderRadius: '20px', padding: '2px 8px', fontSize: '12px', fontWeight: '700'
                  }}>
                    {colTasks.length}
                  </div>
                </div>
                ...
                <div className="kanban-cards">
                  {colTasks.length === 0 ? (
                    <div style={{
                      textAlign: 'center', padding: '20px 16px', color: '#bbb',
                      fontSize: '12px', border: '2px dashed #e5e5e5', borderRadius: '8px', margin: '4px'
                    }}>
                      Drop tasks here
                    </div>
                  ) : (
                    colTasks.map(task => (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                          ...
                          {isOverdue(task) && (
                            <span style={{ background: 'rgba(215,25,32,0.1)', color: '#d71920', padding: '2px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: '700' }}>
                              OVERDUE
                            </span>
                          )}
                        </div>
                        ...
                        {task.client && (
                          <div style={{
                            ...
                            background: 'rgba(215,25,32,0.06)', padding: '2px 8px',
                            borderRadius: '4px', marginBottom: '6px',
                            color: '#d71920', fontSize: '11px', fontWeight: '600'
                          }}>
                            👤 {task.client.name}
                          </div>
                        )}
                          ...
                          <div style={{ marginBottom: '8px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
                              <span style={{ color: '#888', fontSize: '10px' }}>
                                {task.pipeline_stage || 'In Progress'}
                              </span>
                              <span style={{ color: '#d71920', fontSize: '10px', fontWeight: '700' }}>
                  ...
                  {(isAdmin || isManager) && (
                    <button
                      ...
                      style={{
                        ...
                        border: '1px dashed #e5e5e5', borderRadius: '8px',
                        color: '#bbb', cursor: 'pointer', fontSize: '12px',
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <table className="table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '40px', color: '#888' }}>No tasks found</td></tr>
              ) : (
                filtered.map(task => (
            ...
            <div style={{
              ...
              <div style={{ flex: 1, marginRight: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', marginBottom: '10px', flexWrap: 'wrap' }}>
                  ...
                  <span className="badge" style={{
                    background: `${COLUMNS.find(c => c.id === showDetail.status)?.color}15`,
                    color: COLUMNS.find(c => c.id === showDetail.status)?.color
                  }}>
                    {COLUMNS.find(c => c.id === showDetail.status)?.label}
                  </span>
                  {showDetail.client && (
                    <span className="badge" style={{ background: 'rgba(215,25,32,0.08)', color: '#d71920' }}>
                      👤 {showDetail.client.name}
                    </span>
                  )}
                  {isOverdue(showDetail) && (
                    <span className="badge" style={{ background: 'rgba(215,25,32,0.08)', color: '#d71920' }}>OVERDUE</span>
                  )}
                </div>
                ...
                {editingTitle ? (
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <input value={editTitle}
                      ...
                      className="input" style={{ fontSize: '18px', fontWeight: '700' }}
                      autoFocus />
                    <button onClick={updateTitle} className="btn btn-primary btn-sm">Save</button>
                    <button onClick={() => setEditingTitle(false)} className="btn btn-secondary btn-sm">✕</button>
                  </div>
                ) : (
                  <h2
                    style={{ color: '#111', margin: 0, fontSize: '20px', fontWeight: '800', cursor: 'pointer', lineHeight: '1.3' }}
                    onClick={() => { setEditingTitle(true); setEditTitle(showDetail.title) }}
                    title="Click to edit"
                  >
                    {showDetail.title}
                    <span style={{ fontSize: '14px', color: '#ccc', marginLeft: '8px' }}>✏️</span>
                  </h2>
                )}
              </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '20px' }}>
                    {[
                      ...
                      <div key={item.label} style={{ background: '#f9f9f9', borderRadius: '8px', padding: '10px' }}>
                        <div style={{ color: '#bbb', fontSize: '10px', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                          {item.label}
                        </div>
                  ...
                  {showDetail.description && (
                    <div style={{
                      ...
                      <div style={{ color: '#333', fontSize: '14px', lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>
                        {showDetail.description}
                      </div>
                    </div>
                  )}
              ...
              {detailTab === 'pipeline' && (
                <div>
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
                      {pipelineStages.map(stage => (
                        <button key={stage}
                          onClick={() => setPipelineUpdate(prev => ({ ...prev, stage }))}
                          style={{
                            ...
                            fontWeight: pipelineUpdate.stage === stage ? '700' : '500',
                            transition: 'all 0.2s'
                          }}
                        >
                      ...
                      {showNewStage ? (
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <input value={newStage} onChange={(e) => setNewStage(e.target.value)}
                            ...
                            onKeyPress={(e) => e.key === 'Enter' && createPipelineStage()}
                            style={{
                              padding: '7px 12px', border: '1px solid #e5e5e5',
                        ...
                        <button onClick={() => setShowNewStage(true)} style={{
                          padding: '8px 14px', borderRadius: '20px',
                          border: '1px dashed #e5e5e5', background: 'white',
                          color: '#bbb', cursor: 'pointer', fontSize: '13px'
                        }}>
                          + Custom Stage
                        </button>
                      )}
                    ...
                    <div style={{ background: '#f9f9f9', borderRadius: '12px', padding: '20px', marginBottom: '16px', border: '1px solid #e5e5e5' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                        ...
                        <span style={{ color: '#d71920', fontSize: '20px', fontWeight: '800' }}>
                          {pipelineUpdate.percent}%
                        </span>
                      </div>

                      {/* Slider */}
                      <input
                        type="range" min="0" max="100" value={pipelineUpdate.percent}
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginTop: '12px', flexWrap: 'wrap' }}>
                        {[0, 25, 50, 75, 100].map(p => (
                          <button key={p}
                            onClick={() => setPipelineUpdate(prev => ({ ...prev, percent: p }))}
                            style={{
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      <label style={{ color: '#888', fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: '6px' }}>
                        Current Update / Notes
                      </label>
                      <textarea
                        value={pipelineUpdate.note}
                    ...
                    <button onClick={savePipeline} className="btn btn-primary">
                  ...
                  {showDetail.pipeline_stage && (
                    <div style={{
                      background: 'rgba(215,25,32,0.04)', borderRadius: '10px',
                      padding: '16px', border: '1px solid rgba(215,25,32,0.1)'
                    }}>
                      <div style={{ color: '#888', fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>
                        Last Saved Status
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                        <span style={{ color: '#d71920', fontWeight: '700', fontSize: '14px' }}>
                          {showDetail.pipeline_stage}
                        </span>
                        <span style={{
                          ...
                          {showDetail.pipeline_percent}%
                        </span>
                      </div>
                      {showDetail.pipeline_note && (
                ...
                <div>
                  ...
                  {canStartTimer(showDetail) && (
                    <div style={{
                      ...
                      justifyContent: 'space-between', border: '1px solid #e5e5e5'
                    }}>
                      <div>
                        <div style={{ color: '#888', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>
                          Time Tracker
                        </div>
                        <div style={{
                          color: '#d71920', fontSize: '32px',
                          fontWeight: '800', fontFamily: 'monospace'
                        }}>
                          {activeTimer?.task.id === showDetail.id ? formatTimer(timerSeconds) : '00:00:00'}
                        </div>
                        <div style={{ color: '#888', fontSize: '12px', marginTop: '4px' }}>
                          Total logged: {formatDuration(getTotalTime(timeLogs))}
                        </div>
                      </div>
                      <button
                        ...
                        style={{
                          background: activeTimer?.task.id === showDetail.id
                            ? 'rgba(215,25,32,0.08)' : 'rgba(22,163,74,0.08)',
                  ...
                  {timeLogs.length === 0 ? (
                    <div className="empty-state">
                      ...
                      <div className="empty-title">No time logs yet</div>
                      <div className="empty-desc">Start the timer to log work time</div>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      ...
                      {timeLogs.map(log => (
                        <div key={log.id} style={{
                          ...
                          <div style={{
                            color: '#d71920', fontWeight: '800', fontSize: '14px',
                            background: 'rgba(215,25,32,0.08)', padding: '4px 12px', borderRadius: '20px'
                          }}>
                        ...
                        <div key={comment.id} style={{
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              <span style={{ color: '#d71920', fontSize: '13px', fontWeight: '700' }}>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: '560px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {/* Pipeline */}
              <div style={{ background: '#f9f9f9', borderRadius: '10px', padding: '14px', marginBottom: '16px', border: '1px solid #e5e5e5' }}>
                <div style={{ color: '#888', fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '10px' }}>
                  Pipeline (Optional)
                </div>
