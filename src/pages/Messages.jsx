...
export default function Messages({ profile }) {
  ...
  useEffect(() => {
    ...
    const cleanup = subscribeMessages(selectedConvo)
    return cleanup
    ...
    try {
      ...
      if (convo.type === 'dm') {
        res = await supabase
          ...
          .or(`and(sender_id.eq.${profile.id},receiver_id.eq.${convo.id}),and(sender_id.eq.${convo.id},receiver_id.eq.${profile.id})`)
  ...
  const subscribeMessages = (convo) => {
    const subId = `msg-${convo.type}-${convo.id}-${Date.now()}`
    const sub = supabase
      .channel(subId)
      .on('postgres_changes', {
        event: 'INSERT',
        ...
        const relevant = current.type === 'dm'
          ? (
            (payload.new.sender_id === profile.id && payload.new.receiver_id === current.id) ||
            (payload.new.sender_id === current.id && payload.new.receiver_id === profile.id)
          )
          : payload.new.channel_id === current.id
        ...
        if (data) {
          setMessages(prev => {
            ...
            return [...prev.filter(m => !String(m.id).startsWith('temp')), data]
          })
        }
      })
      .subscribe()
  ...
  const createChannel = async () => {
    ...
    if (data) {
      ...
      setSelectedConvo({ ...data, type: 'channel' })
    }
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          {[{ id: 'dms', label: '👤 Direct' }, { id: 'channels', label: '# Channels' }].map(tab => (
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
          {loading ? (
            [1,2,3,4].map(i => <div key={i} className="skeleton" style={{ height: '52px', borderRadius: '10px', marginBottom: '6px' }} />)
          ) : activeTab === 'dms' ? (
            filteredEmployees.length === 0 ? (
              <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '13px' }}>No team members</div>
            ) : filteredEmployees.map(emp => {
              const isSelected = selectedConvo?.id === emp.id && selectedConvo?.type === 'dm'
              const color = roleColor[emp.role] || '#888'
              return (
                <div key={emp.id}
                  onClick={() => setSelectedConvo({ ...emp, type: 'dm' })}
                  style={{
            ...
            <>
              ...
              {filteredChannels.map(channel => {
                ...
                return (
                  <div key={channel.id}
                    onClick={() => setSelectedConvo({ ...channel, type: 'channel' })}
                    style={{
                      display: 'flex', alignItems: 'center', gap: '10px',
                      padding: '10px 12px', borderRadius: '10px', cursor: 'pointer',
                      background: isSelected ? 'rgba(215,25,32,0.08)' : 'transparent',
      ...
      {!selectedConvo ? (
        <div style={{
          ...
          <div style={{ fontSize: '13px' }}>Choose a person or channel</div>
        </div>
      ) : (
        <div style={{
          flex: 1, display: 'flex', flexDirection: 'column',
            ...
            {messages.length === 0 ? (
              <div style={{
                ...
                <div style={{ fontSize: '14px', fontWeight: '700', color: 'var(--text-secondary)' }}>
                  {selectedConvo.type === 'dm'
                    ? `Say hi to ${selectedConvo.full_name}!`
                        ...
                        <div key={msg.id} style={{
                          ...
                          <div style={{
                            ...
                            <div style={{
                              ...
                              boxShadow: isOwn ? '0 3px 10px rgba(215,25,32,0.2)' : 'var(--shadow-sm)',
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  transition: 'all 0.2s',
                  boxShadow: newMessage.trim() ? '0 4px 12px rgba(215,25,32,0.3)' : 'none',
